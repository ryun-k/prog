  ...
  <script type="text/javascript">
  	$(function(){
  		$("#listback").click(function(){
  			location.href="../Member/memberList.do?no=${no}&nowPage=${nowPage}"
  		});
  	});
...
<div class="container">
  ...
  <form class="form-horizontal row" action="" method="post" >
   ...
   <div class="col-12">
	<table class="table table-bordered">
		...
		<tbody align="center">
			<c:forEach var="list" varStatus="status" items="${ALIST}">
				<tr align="center" >
    ...
    <div class="form-group col-12">
      <label class="control-label" for="status">회원등급:</label>
      <div class="col-12">
      	<c:choose>
      		<c:when test="${VIEW.status eq 0}">
		        <input type="text" class="form-control" id="status" name="status" value="탈퇴회원" readonly="readonly">
      		</c:when>
      		<c:when test="${VIEW.status eq 1}">
		        <input type="text" class="form-control" id="status" name="status" value="일반회원" readonly="readonly">
      		</c:when>
      		<c:when test="${VIEW.status eq 9}">
		        <input type="text" class="form-control" id="status" name="status" value="관리자" readonly="readonly">
      		</c:when>
      	</c:choose>
      </div>
    </div>
  ...
  <div class="form-group">        
	    <div class="col-12">
	      <button id="listback" type="submit" class="btn btn-primary btn-block">목록으로</button>
	    </div>
	</div>
</div>
</body>
